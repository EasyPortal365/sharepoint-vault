...
- 💥 **[gotchas/](gotchas/)** — real-world traps as *symptom → cause → fix*
  ...
  - **graph/**
    ...
    - [`perUserMfaState` says "disabled" under Conditional Access](gotchas/graph/per-user-mfa-state-lies-under-conditional-access.md) — the legacy per-user switch stays off while MFA is fully enforced; report on `userRegistrationDetails` instead (and it omits blocked accounts)
