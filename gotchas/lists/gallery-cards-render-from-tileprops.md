...
OData-Version: 4.0          (added by SPHttpClient.configurations.v1)
...
- Send it as plain JSON (`Content-Type: application/json`) with `@odata.type`, not `odata=verbose` with `__metadata` — `SPHttpClient.configurations.v1` adds `odata-version: 4.0`, which a verbose body then collides with (HTTP 400). See [`metadata-body-requires-verbose.md`](../rest-api/metadata-body-requires-verbose.md). The annotation is accepted because of the `OData-Version: 4.0` header, not because of the `Content-Type`: a live A/B test (2026-09-24) found `@odata.type` passing under both `application/json` and `…;odata=nometadata` with that header, and failing under both without it — which is what a bare `fetch` probe sends ([`POST /views` takes an `SP.View` body](../rest-api/creating-a-view-posts-sp-view-not-viewcreationinformation.md)).
- Creating the view from scratch works the same way: `POST /views` with `{ "@odata.type": "#SP.View", "Title": …, "PersonalView": false, "RowLimit": …, "ViewQuery": … }` under the same plain `Content-Type` (or, simpler, with no annotation at all — the recipe measured in the article linked above), then set columns via `POST …/views(guid'…')/viewfields/removeallviewfields` followed by one `POST …/viewfields/addviewfield('<InternalName>')` per column. Those two endpoints take **no body** — send the digest and `Accept` only; an empty JSON body with a `Content-Type` is rejected by some farms.
