...
- Send it as plain JSON (`Content-Type: application/json`) with `@odata.type`, not `odata=verbose` with `__metadata` — `SPHttpClient.configurations.v1` adds `odata-version: 4.0`, which a verbose body then collides with (HTTP 400). See [`metadata-body-requires-verbose.md`](../rest-api/metadata-body-requires-verbose.md). Mind the `Content-Type`: an earlier version of this article showed `application/json;odata=nometadata` here, but the verified code sends plain `application/json` — and under `;odata=nometadata` view writes have been seen rejecting the annotation ([`POST /views` takes an `SP.View` body](../rest-api/creating-a-view-posts-sp-view-not-viewcreationinformation.md)).
