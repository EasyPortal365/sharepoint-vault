...
| [Uploaded `.html` gains mso document properties; Office files are rewritten on upload](rest-api/html-upload-gains-mso-document-properties.md) | Writing column values rewrites the file with `<mso:CustomDocumentProperties>` — SHA-1 read-back fails for markup, holds for PDF/images; `.docx`/`.xlsx`/`.pptx` are rewritten on the upload itself (docProps promotion: 2 170 B → 9 152 B), so their size and hash never match the source |
...
| [`X-RequestDigest` expires mid-session](rest-api/request-digest-expires-mid-session.md) | Writes 403 "security validation is invalid" on a long-open page — raw `fetch` needs a fresh digest from `/_api/contextinfo`; in SPFx a hand-set header switches `SPHttpClient`'s own digest and 403-recovery OFF |
...
| [A list's ETag tracks its schema, so `ItemCount` comes back stale via 304](rest-api/list-metadata-etag-tracks-schema-not-content.md) | Items do not move the list entity's ETag — the browser revalidates, gets 304 and serves the body from when the list was empty; no write of yours is needed for the badge to read 0 over 24 rows |
...
| [PermissionKind is 1-based](permissions/permissionkind-is-one-based-off-by-one-fakes-a-finding.md) | `BasePermissions` bit for kind *k* is `1 << (k - 1)`; decoding with `1 << k` shifts everything by one and reports `ManageLists` where the mask only has `ViewFormPages` — a correctly hardened list then looks wide open |
...
| [A status code measures the order of checks, not permissions](permissions/status-code-measures-the-order-of-checks-not-permissions.md) | a non-destructive probe that writes with a deliberately stale `IF-MATCH` and reads rights off 403 vs 412 is wrong: the etag is evaluated BEFORE authorisation, so 412 comes back even on lists where the account has no `EditListItems`. Ask `EffectiveBasePermissions`, run it under an ordinary account, and print the inputs next to the verdict |
...
| [Protocol-relative URL defeats a host allowlist](spfx/protocol-relative-url-defeats-a-host-allowlist.md) | `//evil.example/Lists/X` matches no `^https?://` check, so an "empty host means relative, relative means our own site" gate lets it through — and the browser completes the scheme and sends the request |
| [A test outside the release command guards nothing](spfx/a-test-outside-the-release-command-guards-nothing.md) | `heft build` does not run Jest, and `prebuild` / `prebuild:ship` drift apart — a rule asserted only in a test never executes on the path the `.sppkg` takes |
| [An unparseable JSON column is not an empty column](spfx/unparseable-json-column-is-not-an-empty-column.md) | `catch → {}` makes "could not read" look like "is empty", and the next save of the record writes the emptiness back — a corrupt theme, vote, thread or targeting rule becomes a blank one; keep three states, throw before the first PATCH, lock the form, recover only by an explicit action |
| [Custom API permission request is "not valid"](spfx/custom-api-permission-request-invalid.md) | API access resolves `resource` to a service principal; create the app registration **before** uploading the `.sppkg`, or the pending request is rejected as a package defect. |
...
| [preAuthorizedApplications needs an existing scope](graph/preauthorized-app-needs-existing-scope.md) | Graph validates `delegatedPermissionIds` against stored scopes, so defining a scope and pre-authorizing it in one `PATCH` returns 400. Split into two calls. |
...
| [A server-side origin check rejects your own smoke test](azure-functions/server-side-origin-check-rejects-your-own-smoke-test.md) | enforcing the allowlist in the handler makes it authorization, not CORS — it applies to curl and PowerShell too, so the deploy script's own smoke test and the runbook's verification command both need the `Origin` header, and "this bypasses CORS" becomes a false claim that fails a healthy deployment |
...
| [`git rm -r` leaves the directory, so tools report a phantom version](tooling/git-rm-leaves-the-directory-so-tools-report-a-phantom-version.md) | `git rm` deletes only tracked files, so untracked build leftovers keep a removed folder alive; a tool listing versions from disk then offers a build the host no longer serves — read `git ls-tree` and fail closed |
