...
- 🔴 **The registration report needs an Entra ID P1/P2 tenant — and the docs don't say so.** On a tenant without a premium license the call fails with:
  ...
  **The licence gate is per-endpoint, not per-feature — verify each call separately.** On the same Entra ID Free tenant where the report returns 403, the beta per-user endpoint works fine: `GET`/`PATCH /users/{id}/authentication/requirements` both succeeded and the change took effect (verified live 2026-07-31). So "no P1" means *you cannot see which methods a user registered*, not *you cannot manage MFA*. Don't disable a whole feature because one of its reads hit a licence wall.
