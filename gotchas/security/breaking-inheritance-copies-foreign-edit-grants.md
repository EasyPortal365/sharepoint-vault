...
>
> **Ve zkratce.** `breakroleinheritance(copyRoleAssignments=true)` zkopíruje na seznam **všechna** oprávnění z webu – včetně skupin, kterým dala `Edit` jiná aplikace. `Edit` nese `ManageLists`, což obchází item-level zabezpečení (`ReadSecurity = 2`), takže si tito lidé přečtou cizí „soukromé" položky. Lam dědičnost bez kopírování a role si přiřaď sám; pak odeber vše, co není na bílé listině.
