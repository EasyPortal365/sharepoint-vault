...
| [Ship SPFx updates without re-uploading .sppkg](runtime-app-versions-without-sppkg-reuploads.md) | Permanent loader webpart + Library component loaded from a versioned CDN folder via `SPComponentLoader.loadComponent` — admin-controlled activate/rollback, retention, and what still needs a package |
| [Provisioning folder trees at scale (measured)](provisioning-folder-trees-at-scale.md) | What creating 86 folders with unique permissions really costs (no throttling), the alias-parameter syntax, reading a whole tree with one item query (two calls) instead of ninety, and `copyRoleAssignments=false` silently dropping the owners group |
