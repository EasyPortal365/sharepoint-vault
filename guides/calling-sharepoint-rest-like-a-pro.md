...
- Text filters on `/_api/web/siteusers` are **case-sensitive** (`substringof('megan', Title)` won't find "Megan") — test the endpoint you rely on rather than assuming one behaviour for all of them. For user-facing search-as-you-type, fetch a candidate set and filter client-side, or use SP Search.
